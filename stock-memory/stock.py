...
if context_total is not None and len(context_total) > 0:
    ...
    try:
        ...
        if API_KEY and raw_df_for_ma is not None and not raw_df_for_ma.empty:
            ...
            ma_class_labels = [
                "💎 [최강 주도주] RS 신고가 + 정배열 완성 (Tier1 매수)",
                "🎯 [VCP 셋업] 변동성 수축 + 거래량 급감 (매수 대기)",
                "🛡️ [지수 대비 강세] KOSPI 하락 중 RS 우상향 (옥석 가리기)",
                "⚡ [추세 재개] 50일선 눌림목 반등 (Power 3) (추가 매수)",
                "🌟 [추세 전환] 1단계 탈피 (Stage 2 진입 초기) (초기 진입)",
                "🔔 [과매수 경계] 200일선 이격 과다 (Climax) (분할 익절)",
                "⚠️ [돌파 실패] 트랩(Trap) 발생 + 20일선 이탈 (리스크 관리)",
                "🏚️ [추세 붕괴] 50일선 하향 돌파 + RS 꺾임 (전량 매도)",
                "💀 [역배열 지속] 장기 이평선 저항 (Stage 4) (접근 금지)",
                "🌫️ [혼조세] 변동성 확대 (Churning) (관망)"
            ]
            vol_class_labels = [
                "🔥 [에너지 분출] 대량거래 돌파 (강력 매수)",
                "📉 [드라이업] 매도세 고갈 (VCP) (매수 대기)",
                "⬆️ [포켓 피벗] 베이스 내 매집 (선취매 기대)",
                "💤 [건전한 휴식] 저거래량 눌림목 (홀팅/추가 매수)",
                "🚨 [분산 발생] 기관의 대량 매도 (위험 신호)",
                "🛡️ [기관 방어] 이평선 지지 거래량 (추세 유효)",
                "🌋 [클라이맥스] 피날레 슈팅 (전량 매도)",
                "⚠️ [허수 반등] 거래량 없는 상승 (추격 금지)",
                "🥊 [쉐이크아웃] 개미 털기 (손절 유도)",
                "🔄 [매물 소화] 고점 박스권 혼조 (관망)"
            ]
            total_class_labels = [
                "💎 [천하무적] 주도주 확정 구간 (최대 비중)",
                "🎯 [VCP 피벗] 폭발 직전 수렴 (매수 주문 대기)",
                "🛡️ [시장 역행] 차기 주도주 후보 (관심 종목 0순위)",
                "🏹 [포켓 피벗] 베이스 내 매집 (선취매 타점)",
                "📈 [추세 강화] 50일선 눌림목 (보유 및 추세)",
                "⚠️ [상승 과열] 클라이맥스 런 (분할 익절)",
                "🧱 [돌파 실패] 트랩(Trap) 경보 (즉시 손절)",
                "📉 [시장 붕괴] 집단 배분(Distribution) (현금 비중 확대)",
                "🧟 [사망 선고] Stage 4 하락 확정 (관심 삭제)",
                "🌊 [바닥 다지기] 1단계 전환 대기 (관찰 리스트)"
            ]
            ...
            system_msg = (
                ...
                "2) 거래량 분류(AI): 거래량 배열과 추세 등을 기준으로 아래 [거래량 분류표] 중 1개 선택. 단, 거래량(현재)는 판단에서 제외\n"
...
if prompt := st.chat_input("질문해보세요!"):
    ...
    with st.chat_message("assistant"):
        ...
        system_prompt = (
            "당신은 마크 미너비니, 윌리엄 오닐을 존경하고 따르는 수제자로써, 트렌드 템플릿, CAN SLIM, VCP(변동성 수축 패턴), RS 등급(Relative Strength Rating), 거래량 드라이업(Volume Dry-up), 기관 매집/분산(Accumulation/Distribution) 원칙을 배웠으며, 그 결과, 추세 추종 돌파 매매 및 VCP 관점으로 하는 전문 주식 투자자이다. 아래 objects / RAW data를 참고하여 질문에 답변하고 조언을 하시오. "
