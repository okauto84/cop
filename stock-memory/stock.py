...
if context_total is not None and len(context_total) > 0:
    ...
    try:
        ...
        if API_KEY and raw_df_for_ma is not None and not raw_df_for_ma.empty:
            ...
            system_msg = (
                ...
                "5) 투자관점 정리(AI): 해당 종목에 대한 현재의 뉴스 기사, 시장 상황, KOSPI 지수, RS 지수 등과 RAW_Table 시계열 데이터(날짜, 종가, 이동평균선, 거래량)를 함께 종합하여 분석하여 투자(매매)관점으로 액션에 대해서 300자 수준으로 요약 (한글 기준, 공백·기호 포함 150자 수준)\n\n"
    ...
    # Objects를 단일 표 형식으로 출력 (접고/펼치기 카드 제거)
